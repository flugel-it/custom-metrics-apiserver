...
docker-build: vendor
	...
	docker run -it -v $(TEMP_DIR):/build -v $(shell pwd):/go/src/github.com/kubernetes-incubator/custom-metrics-apiserver -e GOARCH=$(ARCH) golang:1.8 /bin/bash -c "\
		CGO_ENABLED=0 go build -a -tags netgo -o /build/sample-adapter github.com/kubernetes-incubator/custom-metrics-apiserver"
